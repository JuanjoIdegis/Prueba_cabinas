...
document.addEventListener("DOMContentLoaded", async () => {
  // Limpiar rastros antiguos de GitHub del almacenamiento local del navegador
  try {
    localStorage.removeItem("cabina_github_token");
    localStorage.removeItem("cabina_github_owner");
    localStorage.removeItem("cabina_github_repo");
    const oldBase = localStorage.getItem("cabina_base_url");
    if (oldBase && (oldBase.includes("github.io") || oldBase.includes("github.com"))) {
      localStorage.removeItem("cabina_base_url");
    }
  } catch (e) {}
...
async function saveSlotForm() {
  ...
  try {
    ...
    if (syncResult && syncResult.success) {
      showToast(`✅ ${targetSlotId} guardado correctamente`, "success");
    } else if (syncResult && syncResult.error) {
      showToast(`⚠️ Guardado local: ${syncResult.error}`, "warning");
    } else {
...
async function confirmarFinalizarPrueba(slotId) {
  ...
  if (confirm(confirmMsg)) {
    ...
    await Store.finalizarPruebaSlot(slotId);
...
async function confirmarLiberar(slotId) {
  ...
  if (confirm(`¿RETIRAR y dejar totalmente VACÍO el puesto ${slotId} (${nombreEquipo})?\n\n• El equipo y fotos se archivarán en el Histórico.\n• El puesto quedará LIBRE para conectar otro equipo nuevo desde cero.`)) {
    await Store.liberarSlot(slotId);
    renderPuestos();
    updateMetrics();
    showToast(`✅ ${slotId} retirado y archivado en el Histórico (Puesto Libre)`, "warning");
  }
}
...
async function ejecutarMoverEquipo() {
  ...
  try {
    await Store.moverEquipo(moverFromSlotId, moverToSlotId);
    closeMoverModal();
...
function ejecutarOpenMobileConnectModal() {
  ...
  let url = "https://juanjoidegis.github.io/Prueba_cabinas";
  if (window.location.hostname.includes("github.io")) {
    url = window.location.href.split("#")[0].split("?")[0].replace(/\/$/, "");
  }
  ...
  if (badgeEl) {
    badgeEl.textContent = "🌐 Acceso Cloud GitHub Pages (Global)";
  }
...
function ejecutarOpenSyncModal() {
  ...
  if (urlInput) {
    urlInput.value = QRGenerator.getBaseUrl() || "https://juanjoidegis.github.io/Prueba_cabinas";
  }
}
...
async function sincronizarConCloudAhora() {
  showToast("☁️ Sincronizando con GitHub Cloud...", "info");
  try {
    const res = await Store.saveToGitHub("Sincronización manual desde la app");
    if (res && res.github) {
      showToast("✅ ¡Datos y fotos sincronizados con GitHub Cloud!", "success");
      await Store.fetchData();
      renderApp();
    } else {
      showToast(`⚠️ Guardado: ${res.error || 'comprueba conexión'}`, "warning");
    }
  } catch (err) {
    showToast(`⚠️ Error al sincronizar: ${err.message}`, "warning");
  }
}
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (parsed.slots) {
        ...
        showToast("✅ Base de datos importada y guardada en el servidor local.", "success");
      } else {
